...
\[
\mathcal{M}=(\Sset,\A,\Oset,\T,\mathcal{Z},\R,\gamma)
\]
such that a hidden layout $B$ is sampled once at time $0$ and then fixed; the public state evolves deterministically through the hit, miss, and sunk updates; observations are deterministic shot outcomes conditional on the hidden layout and public shot record; and the default reward is a step penalty until all ships are sunk.
\end{definition}
...
\min_{\pi}\E_{\pi}[\tauT].
\]
The following lemma, which is standard but operationally important, explains why the sparse step-penalty reward is the principled default.

\begin{lemma}[Step penalty and shots-to-win]
Suppose the training reward is
\[
r_t=-1 \quad \text{for } t<\tauT, \qquad r_{\tauT}=0.
\]
Then for any policy $\pi$,
\[
J(\pi):=\E_{\pi}\left[\sum_{t=0}^{\tauT-1}r_t\right] = -\E_{\pi}[\tauT].
\]
Therefore maximizing expected return is exactly equivalent to minimizing expected shots-to-win.
\end{lemma}
...
\pi:\Hset\to\A
\]
that assigns a legal action to each feasible history.
\end{definition}
...
\begin{definition}[Attacker $\varepsilon$-best response to a mixture]
Fix an attacker policy class $\Pi$ and let
\[
\mu=\lambda \rho_D+(1-\lambda)\rho_U, \qquad \lambda\in(0,1).
\]
A policy $\pi^\varepsilon\in\Pi$ is an $\varepsilon_A$-best response to $\mu$ if
\[
\E_{\mu}[\tauT(\pi^\varepsilon)]-\inf_{\pi\in\Pi}\E_{\mu}[\tauT(\pi)]\le \varepsilon_A.
\]
\end{definition}
...
\[
\mu_k=\lambda \rho_k+(1-\lambda)\rho_U,
\]
then
\[
...
These inequalities are the formal reason the Stage-2 diagnostics are useful. A negative \textsf{defender\_adversarial} is not merely an undesirable scalar; it is evidence that the defender did not reach an adequate best response under the current optimization budget or representational constraints. Likewise, a positive weighted residual involving \textsf{attacker\_adaptation} and \textsf{uniform\_drift} indicates that the attacker phase failed to sufficiently optimize the stated mixture objective.
...
An analogous bound holds for the weighted attacker residual
\[
\lambda \cdot \textsf{attacker\_adaptation}_k + (1-\lambda)\cdot \textsf{uniform\_drift}_k.
\]
\end{theorem}
...
There exist adversarial latent-initial-state POMDPs, two defender distributions $\rho$ and $\rho'$ with identical one-coordinate marginals over the latent variable, and a deterministic attacker policy $\pi$ such that
\[
\E_{\rho}[\tauT(\pi)] \neq \E_{\rho'}[\tauT(\pi)].
\]
...
The empirical diagnostics are therefore not ad hoc. The quantity \textsf{defender\_adversarial} operationalizes the defender certificate theorem. The weighted combination of \textsf{attacker\_adaptation} and \textsf{uniform\_drift} operationalizes the attacker certificate theorem. The finite-sample theorem explains when the empirical signs of these quantities are trustworthy. This is the conceptual bridge from theory to the tables and curves in the later sections. The certificate theorems should therefore be read as \emph{optimization diagnostics for empirically estimated quantities}, not as claims that every observed sign pattern must hold exactly at finite budget and finite sample size.
...
The solution-space choices are now easy to state. The reported experiments use the sparse step-penalty reward as the baseline reward, because it is exactly aligned with shots-to-win. A shaped reward family is conceptually possible and is discussed in the solution-space specification, but it is not the focus of the reported results. The attacker policy actually implemented for the main experiments is a feedforward masked PPO policy over the public three-channel board tensor rather than a recurrent explicit-belief controller. This is a methodological choice rather than a property of the problem itself. The baseline self-play protocol is alternating optimization: hold the defender fixed while training the attacker, then hold the attacker fixed while training the defender. More elaborate game-theoretic stabilizers are possible, but they are not needed to define the present benchmark.
...
The Stage-2 results are more nuanced and, in some ways, more informative. Table~\ref{tab:ibr} gives the final three-seed iterative best-response diagnostics now contained in the uploaded paper. The quantity \textsf{defender\_adversarial} is the key empirical bridge to the theory. Positive values indicate that the learned defender is harder than the nominal UNIFORM defender for the current attacker. Under exact best response, Theorem 2 says this quantity cannot be negative. In the table, it is sometimes positive and sometimes negative. This is exactly the optimization-sensitive behavior the theorem package is meant to diagnose. The negative entries do not undermine the formulation. Rather, they suggest that the defender phase has not reached a sufficiently good best response under the current budget or defender class.

\begin{table}[t]
\centering
\caption{Final Stage-2 iterative best-response diagnostics across three independent seeds. Defender budgets are restricted to 50k steps per generation to simulate a computationally constrained adversarial threat model.}
\label{tab:ibr}
\resizebox{\linewidth}{!}{
\begin{tabular}{cccccccc}
\toprule
Seed & Generation ($k$) & UNIFORM & SPREAD & $D_k$ & defender\_adversarial & attacker\_adaptation & uniform\_drift \\
\midrule
...
The Stage-2 table also clarifies the third experimental question. The attacker does adapt in some generations, but that adaptation is not uniformly clean. The correct way to read the table is not to ask whether \textsf{attacker\_adaptation} is always negative in isolation. Theorem 2 says that the correct object is the \emph{weighted combination} of attacker adaptation and nominal drift. In other words, a small amount of nominal degradation can still be acceptable if it is compensated by sufficient improvement against the learned defender. The empirical picture is therefore not one of failure, but of partial success under a budget-limited adversarial regime.
...
The second limitation concerns the defender side of the game. The theoretical statements are exact under exact best responses and become approximate under $\varepsilon$-best responses. The experiments strongly suggest that the defender is often the optimization bottleneck. This means that Stage-2 failures should not be over-read as failures of the minimax formulation itself. At the same time, it means the empirical IBR evidence is only as convincing as the defender optimization budget and class are strong.
...
This paper introduced adversarial latent-initial-state POMDPs as a restricted but mathematically tractable class of robustness problems. For this class, it proved a latent minimax principle, an extreme-point characterization of worst-case defenders, approximate best-response certificates, finite-sample sign-certification results, and a structural marginal-insufficiency proposition. These results are modest in scope, but they serve a clear purpose: they give exact mathematical meaning to the diagnostics used during adversarial latent-state training.
...
$\lambda$ & Mixture weight on the defender-induced distribution in Stage 2. \\
...
Because the horizon $T_{\max}$ is finite and both the action and observation alphabets are finite, the number of feasible attacker histories is finite. A deterministic history-dependent attacker policy is a map from this finite history set into the finite action set. Therefore the number of deterministic history-dependent attacker policies is finite. Denote this finite set by $\Pi^{\det}$.
...
\mu_k=\lambda \rho_k+(1-\lambda)\rho_U.
\]
By definition, $\pi_k$ is an $\varepsilon_A$-best response to $\mu_k$ if
\[
\E_{\mu_k}[\tauT(\pi_k)]-\inf_{\pi\in\Pi}\E_{\mu_k}[\tauT(\pi)]\le \varepsilon_A.
\]
Since $\pi_{k-1}\in\Pi$, we have
\[
\inf_{\pi\in\Pi}\E_{\mu_k}[\tauT(\pi)]\le \E_{\mu_k}[\tauT(\pi_{k-1})].
\]
Therefore
\[
\E_{\mu_k}[\tauT(\pi_k)]\le \E_{\mu_k}[\tauT(\pi_{k-1})]+\varepsilon_A.
\]
Expanding the definition of $\mu_k$,
\[
...
For the weighted attacker residual, write the empirical quantity as a fixed linear combination of four independent empirical means,
\[
\widehat{R}_k
=
\lambda(\overline{X}_1-\overline{X}_2)+(1-\lambda)(\overline{Y}_1-\overline{Y}_2),
\]
where each empirical mean is again formed from bounded i.i.d. episode lengths in $[0,T_{\max}]$. Apply Hoeffding's inequality separately to each empirical mean, choose each deviation bound so that its failure probability is at most $\delta/4$, and apply a union bound. The same algebra as above gives the stated weighted confidence radius.
...
\begin{figure}[h!t]
    ...
    \caption{Training dynamics and IBR diagnostics averaged over independent seeds. \textbf{(a)} Fixed-mixture and alternating stress approaches readily acquire nominal proficiency without catastrophic gameplay degradation. \textbf{(b)} Direct targeted adversarial exposure effectively limits extreme worst-case trajectory vulnerabilities, explicitly lowering tail severity ($\cvar_{0.10}$ and $p95$). \textbf{(c)} In Stage 2, tracked IBR metrics confirm sufficient optimization bandwidth yields positive \textsf{defender\_adversarial} shifts that provoke direct \textsf{attacker\_adaptation}, fulfilling the theoretical diagnostic certificates mapping to the latent minimax formulation.}
